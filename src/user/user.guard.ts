 ...
 * @LastEditTime: 2023-11-27 14:45:29
 * @LastEditors: wurangkun
 * @FilePath: \fontend-for-personal\src\user\user.guard.ts
 * @Description: 
 */
...
export class UserGuard implements CanActivate {
  ...
  canActivate(
    ...
    const token = headers.session_id
    if (token) {
      //验证token或解码获取信息
      try {
        ...
        if (user?.openId) {
          ...
          if (this.userService.blackSet.has(user.openId)) {
